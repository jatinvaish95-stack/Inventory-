...
function processCommand(){
  ...
  let qtyMatch=n.match(
    /(\d+(?:\.\d+)?)\s*(?:gadde|gadda|pcs|piece|pieces|unit|units|नग|गद्दे|गद्दा)/i
  );

  let qty=qtyMatch
    ? Number(qtyMatch[1])
    : Number((n.match(/\b(\d+)\b/)||[])[1]||1);

  const p=findProduct(raw);

  if(!p){
    $("status").textContent="Product not found. Add it first.";
    return;
  }

  const isOut=
    /\b(bik|bicke|biki|sell|sold|out|remove|nikal|gaye|gaya)\b/
    .test(n)
    ||
    /बिक|बेच|बिके|गये|गया|निकाल/.test(n);
  ...
  if(isOut){
    ...
    $("status").textContent=
      `Stock Out: ${qty} ${p.unit} from ${p.product}.`;
    ...
    $("status").textContent=
      `Stock In: ${qty} ${p.unit} to ${p.product}.`;
  }
